...
// common default optiond for both useQuery and preFetchQuery
const commonDefaultOpts = {
  staleTime: 0,
  cacheTime: 300000,
}
...
export function useAppointments(): UseAppointments {
  ...
  useEffect(() => {
    const nextMonthYr = getNewMonthYear(monthYear, 1)
    queryClient.prefetchQuery({
      queryKey: [`${queryKeys.appointments}`, nextMonthYr.year, nextMonthYr.month],
      queryFn: () => getAppointments(nextMonthYr.year, nextMonthYr.month),
      ...commonDefaultOpts
    })
